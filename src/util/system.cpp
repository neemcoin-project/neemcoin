...
const char * const BITCOIN_CONF_FILENAME = "neemcoin.conf";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Neemcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Neemcoin";
#else
    // Unix
    return pathRet / ".neemcoin";
#endif
#endif
}
...
{
    std::string strYearn = strPrefix;
    strYearn.replace(strYearn.find("2011-2020"), sizeof("2011-2020")-1, "2021");

    const auto copyright_devs = strprintf(_(COPYRIGHT_HOLDERS), COPYRIGHT_HOLDERS_SUBSTITUTION);
    //std::string strCopyrightHolders = strPrefix + copyright_devs;
    std::string strCopyrightHolders = strYearn + copyright_devs;
