# PSBT Howto for Neemcoin Core

Since Neemcoin Core 0.17, an RPC interface exists for Partially Signed Neemcoin
...
PSBT is an interchange format for Neemcoin transactions that are not fully signed
...
Overall, the construction of a fully signed Neemcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Neemcoin transaction (in network format)
...
#### Multisig with multiple Neemcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Neemcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `neemcoin-cli` is used.
